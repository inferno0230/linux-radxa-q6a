...
pkgbase=linux-radxa-q6a
pkgname=("${pkgbase}" "${pkgbase}-headers")

pkgdesc="Linux kernel for Radxa Q6A"
url='https://github.com/inferno0230/linux-radxa-q6a'
arch=(aarch64)
...
_package() {
    ...
    install -Dm644 "${image_path}" "${modulesdir}/vmlinuz"
    ...
    if [ -d "${OUT_DIR}/dtbinst/qcom" ]; then
        ...
        mkdir -p "${pkgdir}/boot/dtb/qcom"
        cp -a "${OUT_DIR}/dtbinst/qcom/"* "${pkgdir}/boot/dtb/qcom/"
    else
